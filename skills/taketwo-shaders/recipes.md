...
Reference Claude reads when `taketwo-shaders` is invoked. Each recipe cites real `file:line` from the kit + site so it ages with the codebase.
...
Driven by the canvas-dim ScrollTrigger (`HomeScene.ts:401-412`). Material has `transparent: true` set in `HeroSculpture.ts`. See `taketwo-scroll` recipes §5 for the trigger pattern.
...
1. **Confirm against the slop checklist** in `taketwo-threejs` skill. Custom shader is the answer to "no MeshBasicMaterial / MeshStandardMaterial on hero elements."
...
- Ambient particle fields (covered by `taketwo-threejs` and `brain/design-taste.md`).
...
- **Material doesn't fade on scroll.** Forgot `transparent: true` on the material, OR `uAlpha` uniform isn't wired to the scroll trigger. See `taketwo-scroll` recipes §5 for the canvas-dim trigger.
- **`?raw` import returns undefined.** `optimizeDeps.exclude: ['@taketwo/kit']` missing in `astro.config.ts`. Required for the kit's `?raw` consumers to work — without it esbuild pre-bundling chokes on the import syntax.
